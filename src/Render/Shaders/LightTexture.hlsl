...
float3 ComputePhongSpecular(float3 lightDirection, float3 viewDirection, float3 normal, float3 lightColor, float4 materialSpecular, float materialShininess)
{
...
float3 ComputeBlinnPhongSpecular(float3 lightDirection, float3 viewDirection, float3 normal, float3 lightColor, float4 materialSpecular, float materialShininess)
{
    float3 halfwayDir = normalize(lightDirection + viewDirection);
    float specularFactor = pow(max(dot(normal, halfwayDir), 0.0f), materialShininess);
    return lightColor * materialSpecular.rgb * specularFactor;
}
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    // Calculer la couleur spéculaire selon le modèle Phong
    float3 viewDirection = normalize(-pin.PosH.xyz); // Vue direction
    float3 specularColorPhong = ComputePhongSpecular(lightDirection, viewDirection, normal, lightColor, materialSpecular, materialShininess);

    // Calculer la couleur spéculaire selon le modèle Blinn-Phong
    float3 specularColorBlinnPhong = ComputeBlinnPhongSpecular(lightDirection, viewDirection, normal, lightColor, materialSpecular, materialShininess);
