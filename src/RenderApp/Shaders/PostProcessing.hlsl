...
// Fonction pour calculer l'intensité du bloom en fonction des coordonnées UV et de l'identifiant d'objet
...
float4 PS(VSOutput pin) : SV_Target
{
    ...
    // Échantillonnez l'identifiant d'objet de la texture
    ...
    if (objectId >= 99 && objectId <= 102)
    {
        ...
        // Appliquer le bloom à l'échantillon de couleur
        sampledColor.rgb += bloomIntensity;
    }
    else
    {
        // Ne pas appliquer le bloom aux pixels qui ne satisfont pas la condition
        // Vous pouvez laisser cette partie vide ou retourner simplement l'échantillon de couleur sans modification
    }
